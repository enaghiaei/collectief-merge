...
exports.update_ = function (schedule, res0) {
   //console.log(schedule)
    ...
    var sql = "update notification set nt_deleted='1' where nt_id = ?";
    var where = [schedule.id];
    con.query(sql, where, function (err, result) {
        if (err) throw err;
        updated = result.affectedRows;
        if (updated > 0) {
            var res2 = con.query("select nt_id AS id FROM notification where nt_deleted =0 and nt_user=?", [0], function (err, result, fields) {
               //console.log(result);
