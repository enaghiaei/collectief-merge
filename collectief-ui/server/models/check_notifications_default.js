    ...
    for (var key in result0) {
       ...
       //console.log("key", key)
        ...
        for (var key2 in result1) {
            ...
            if (parseInt(result0[key].operation) == 0) {
                if (result1[key2].cl_title != "" && result1[key2].cl_title != null)
                    message = result0[key].measure_name + " of " + result1[key2].cl_title + " is LOW ";
                else
                    message = result0[key].measure_name + " of " + result1[key2].sensor_serial + " is LOW ";
            } else if (parseInt(result0[key].operation) == 1) {
                message = ""
            }
            else if (parseInt(result0[key].operation) == 2) {
                //message = result0[key].measure_name + " is HIGH ";
                //message = result0[key].measure_name + " is HIGH ";
                if (result1[key2].cl_title != "" && result1[key2].cl_title != null)
                    message = result0[key].measure_name + " of " + result1[key2].cl_title + " is HIGH ";
                else
                    message = result0[key].measure_name + " of " + result1[key2].sensor_serial + " is HIGH ";
            }
           ...
           //console.log("message", message)
            var [result4, fields4] = await con.execute("SELECT * FROM notification_messages  WHERE nm_message = '" + message + "'  AND (" + time_now + " - nt_date) < '" + time + "'");
            var [result5, fields5] = await con.execute("SELECT * FROM notification_messages  WHERE nm_message = '" + message + "'");
            var vals = result0[key].nt_id + ",'" + result1[key2].av + "','" + message + "','" + Math.floor(new Date().getTime() / 1000) + "','" + result1[key2].sensor_serial + "'";
            if (result4.length == 0 && result5.length == 0) {
                [result7] = await con.execute("INSERT INTO notification_messages (nt_id,nm_value,nm_message,nt_date,nm_sensor_id) VALUES (" + vals + ")");
            } else if (result4.length == 0){               
                [result6] = await con.execute("update notification_messages set nm_seen = 0 , nm_deleted = 0 , nt_date = " + time_now +" where nm_message = '" + message + "'");
            }
