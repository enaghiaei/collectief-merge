...
async function get_data_per_hour(token, res0) {
    ...
    */
    ...
    if (m < 10) {
        ...
        var date3 = new Date(today_time - (1000 * 60 * (m + 1 + 50)));
    ...
    if (today42 < 10) {
        today42 = "0" + today42;
           ...
           ////console.log("today_tmp", today_tmp)
            ...
            // inja bayad begim age baraye 15 min or 30 min ghabli bood va status oon 0 bood age 1 bood ke yaeni khodesh tolid shode va vagheei nist pas 0 mishe
   ...
   //console.log("x1:", x3);
       ...
       ////console.log(sql)
        [rows4,fields4] = await con.execute("select * FROM node_per_hour  WHERE" + " nph_from2 = '" + x0 + "' AND nph_to2 = '" + x1 + "'");
    try {
    ...
    try {
        [rows8, fields8] = await con.execute("SELECT * FROM node_per_hour WHERE nph_to2 >= '" + x3 + "'");
    } catch (error) {
        console.log(error)
    }
                   ...
                   //console.log("xxxxxxxxxxxx")
                    if (rows.length > 0) {
                        for (var key in rows) {
                            ...
                            try {
                                [rows2] = await con.execute("INSERT INTO node_per_hour (sensor_serial,nph_max,nph_min,nph_avg,nph_kind,nph_name,nph_from,nph_to,nph_from2,nph_to2) VALUES (" + vals + ")");
                            } catch (error) {
                                console.log(error)
                            }
                        }
                        ...
                        var nph_avg = 0;
                        var nph_status = 0;
                        for (var key8 in rows8) {
                            if (rows5[key5]["sl_sensor"] == "22040207") {
                                //console.log("%%%%%%%%%%%%%%%%%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&%%%%%%%%%%%%%%%%%%%%%%%%%")
                                //console.log(rows8[key8]["measure_kind"])
                            }
                            if (rows8[key8]["sensor_serial"] == rows5[key5]["sl_sensor"] && rows8[key8]["nph_kind"] == rows6[key6]["measure_kind"] && rows8[key8]["nph_name"] == rows6[key6]["measure_name"] && rows8[key8]["nph_status"] == 0 && rows8[key8]["nph_max"] != 0) {
                                nph_status = 1;
                                nph_max = rows8[key8]["nph_max"];
                                nph_min = rows8[key8]["nph_min"];
                                nph_avg = rows8[key8]["nph_avg"];
                            }
                        }
...
module.exports = {
    get_data_per_hour: get_data_per_hour
}
