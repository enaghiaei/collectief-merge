       ...
       //console.log("key", key)
        ...
        for (var key2 in result1) {
           ...
           //console.log("message", message)
            [result4, fields4] = await con.execute("SELECT * FROM notification_messages  WHERE nm_message = '" + message + "'  AND (" + time_now + " - nt_date) < '" + time + "'");
            var vals = result0[0].nt_id + ",'" + result1[key2].av + "','" + message + "','" + Math.floor(new Date().getTime() / 1000) + "','" + result1[key2].sensor_serial + "'";
            if (result4.length == 0) {
                [result5] = await con.execute("INSERT INTO notification_messages (nt_id,nm_value,nm_message,nt_date,nm_sensor_id) VALUES ("+vals+")");
            }
