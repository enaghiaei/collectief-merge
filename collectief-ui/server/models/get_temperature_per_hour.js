...
async function get_temperature_per_hour(token, res0) {
    ...
    */
    ...
    for (var i = 1; i > 0; i--) {
           ...
           ////console.log("today_tmp", today_tmp)
            ...
            var x1 = today3_2 + "-" + today1_2 + "-" + today2_2 + " " + h_2 + ":" + "59" + ":" + "00";
           ////console.log("x0:", x0);
            //console.log("x1:", x1);
       ...
       ////console.log(sql)
        ...
        [rows, fields] = await con.execute("SELECT nph_avg AS av,nph_max AS av_max,nph_min AS av_min,nph_from AS range_title,nph_name AS measure_name,cl_id FROM node_per_quarter JOIN collectief_assignment ON collectief_assignment.sensor_id = node_per_quarter.sensor_serial  WHERE nph_kind = 'Temperature'" + " AND nph_name = 'Air temperature' AND ca_deleted=0 AND nph_from2 >= '" + x0 + "'" + " AND nph_from2 <= '" + x1 + "'");
...
module.exports = {
    get_temperature_per_hour: get_temperature_per_hour
}
