...
async function get_pressure_per_hour(token, res0) {
    ...
    */
    ...
    for (var i = 1; i > 0; i--) {
        ...
        [rows, fields] = await con.execute("SELECT nph_avg AS av,nph_max AS av_max,nph_min AS av_min,nph_from AS range_title,nph_name AS measure_name,cl_id FROM node_per_quarter JOIN collectief_assignment ON collectief_assignment.sensor_id = node_per_quarter.sensor_serial  WHERE nph_kind = 'Pressure'" + " AND ca_deleted=0  AND nph_from2 >= '" + x0 + "'" + " AND nph_from2 <= '" + x1 + "' ");
        for (var key in rows) {
