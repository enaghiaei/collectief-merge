...
async function get_(token, res0) {
    ...
    /* var res1 = con.query("select * FROM session where s_token=?", [token.token], function (err, result0, fields) {
           //console.log(result0);
            if (result0[0]) {*/
    var res = con.query("select nt_value AS value,nt_type AS type,nt_operation AS operation,nt_id AS id FROM notification_base where  nt_deleted =0 and  nt_user=?", [0], function (err, result, fields) {
       //console.log(result);
