...
exports.power_ = function (schedule, res0) {
   //console.log(schedule)
    ...
    var res1 = con.query("select * FROM session where s_token=?", [schedule.token], function (err, result0, fields) {
        ...
        var sql = "select * FROM schedule where sc_id = ?";
        con.query(sql, where, function (err, result1) {
            if (result1[0]["sc_active"] == 1)
                var sql = "update schedule set sc_active='0' where sc_id = ?";
            else
                var sql = "update schedule set sc_active='1' where sc_id = ?";
    var where = [schedule.id];
