    ...
    */
    ...
    for (var i = 1; i > 0; i--) {
       ...
       // [rows, fields] = await con.execute("SELECT AVG(measure_value) AS av,MAX(measure_value) AS av_max,MIN(measure_value) AS av_min,'" + h + " to " + h_2 + "' AS range_title,cl_id,sensor_serial FROM node_2204036" + num_t + " JOIN collectief_assignment ON collectief_assignment.sensor_id = node_2204036" + num_t + ".sensor_serial  WHERE measure_kind = 'Humidity'" + " AND timestamp >= '" + x0 + "'" + " AND timestamp <= '" + x1 + "' GROUP BY cl_id,measure_name");      
        [rows, fields] = await con.execute("SELECT nph_avg AS av,nph_max AS av_max,nph_min AS av_min,nph_from AS range_title,nph_name AS measure_name,sensor_serial FROM node_per_quarter  WHERE nph_kind = 'Humidity'" + " AND nph_from2 >= '" + x0 + "'" + " AND nph_from2 <= '" + x1 + "'");
