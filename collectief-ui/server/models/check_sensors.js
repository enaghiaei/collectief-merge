    ...
    for (var key in rows2_0) {
       ...
       //console.log("status", rows2_0[key].sl_status)
        if (rows2_0[key].sl_status == 1) {
           //console.log("in0")
            await con.execute("UPDATE notification_messages SET nm_archived = 1 WHERE nm_type = 1 and nm_sensor_id = ?", [rows2_0[key].sl_sensor]);
            await con.execute("UPDATE notification_messages SET nm_archived = 0 WHERE nm_type = 0 and nm_sensor_id = ?", [rows2_0[key].sl_sensor]);
        }
        else {
           //console.log("in1")
            await con.execute("UPDATE notification_messages SET nm_archived = 1 WHERE nm_type = 0 and nm_sensor_id = ?", [rows2_0[key].sl_sensor]);
        }
    }
   //console.log(rows);
    if (rows0.length > 0) {
