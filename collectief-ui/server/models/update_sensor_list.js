...
async function update_sensor_list(token, res0) {
   ...
   //console.log("today_tmp", today_tmp)
    ...
    //else {
    [rows, fields] = await con.execute("SELECT DISTINCT sensor_serial FROM node_220403690", [])
   //console.log("unique sensors1", rows)
    for (var key in rows) {
...
module.exports = {
    update_sensor_list: update_sensor_list
}
