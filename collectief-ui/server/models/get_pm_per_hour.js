...
async function get_pm_per_hour(token, res0) {
    ...
    */
    ...
    for (var i = 1; i > 0; i--) {
       ...
       // [rows, fields] = await con.execute("SELECT AVG(measure_value) AS av,MAX(measure_value) AS av_max,MIN(measure_value) AS av_min,'" + h + " to " + h_2 + "' AS range_title,measure_name,cl_id FROM node_2204036" + num_t + " JOIN collectief_assignment ON collectief_assignment.sensor_id = node_2204036" + num_t + ".sensor_serial  WHERE measure_kind = 'Mass'" + " AND timestamp >= '" + x0 + "'" + " AND timestamp <= '" + x1 + "' GROUP BY cl_id,measure_name");        for (var key in rows) {
        [rows, fields] = await con.execute("SELECT nph_avg AS av,nph_max AS av_max,nph_min AS av_min,nph_from AS range_title,nph_name AS measure_name,cl_id FROM node_per_quarter JOIN collectief_assignment ON collectief_assignment.sensor_id = node_per_quarter.sensor_serial  WHERE nph_kind = 'Mass'" + " AND ca_deleted=0  AND nph_from2 >= '" + x0 + "'" + " AND nph_from2 <= '" + x1 + "'");
        for (var key in rows) {
...
module.exports = {
    get_pm_per_hour: get_pm_per_hour
}
