...
async function get_sensor_data(token,res0) {
   ...
   //console.log("today_tmp", today_tmp)
    ...
    //else {
    [rows, fields] = await con.execute("SELECT max(nph_from2) AS nph_f FROM node_per_quarter where sensor_serial='" + token.serial + "'");
   //console.log("rows",rows)
    if (rows[0])
        [rows2, fields2] = await con.execute("SELECT nph_kind AS measure_kind,nph_name AS measure_name,nph_avg AS measure_value,sensor_serial  FROM node_per_quarter where sensor_serial = '" + token.serial + "' AND nph_from2 = '" + rows[0]["nph_f"] + "'");
    else {
        rows2 = [];
    }
   // }
