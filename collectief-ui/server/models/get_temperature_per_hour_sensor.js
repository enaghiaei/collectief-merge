...
async function get_temperature_per_hour_sensor(token, res0) {
    ...
    */
    ...
    for (var i = 1; i > 0; i--) {
       ...
       ////console.log(sql)
        ...
        [rows, fields] = await con.execute("SELECT nph_avg AS av,nph_max AS av_max,nph_min AS av_min,nph_from AS range_title,nph_name AS measure_name,sensor_serial FROM node_per_quarter WHERE nph_kind = 'Temperature'" + " AND nph_name = 'Air temperature' AND nph_from2 >= '" + x0 + "'" + " AND nph_from2 <= '" + x1 + "' ORDER BY nph_from2 ASC");
...
module.exports = {
    get_temperature_per_hour_sensor: get_temperature_per_hour_sensor
}
