               ...
               //console.log("result",result)
                if (result[0]["user_type"] == 4) {
                   //console.log(result);
                    ...
                    var res = con.query("select users.*,location_detail FROM users LEFT JOIN users_detail on users.user_id = users_detail.user_id LEFT JOIN collectief_location ON collectief_location.cl_id = location_id WHERE (location_id=? OR cl_parent=?)", [result[0].location_id, result[0].location_id], function (err, result2, fields) {
                       //console.log(res.sql)
                       //console.log(result);
                    ...
                    var res = con.query("select users.*,location_detail FROM users LEFT JOIN users_detail ON users.user_id = users_detail.user_id WHERE user_type != 0", [], function (err, result2, fields) {
                       //console.log(res.sql)
                       //console.log(result);
                        res0.json({ result: result2 });
                        con.end();
