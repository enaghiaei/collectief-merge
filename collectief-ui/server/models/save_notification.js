...
exports.save_notification = function (schedule, res0) {
   ...
   //console.log(global.config)
    ...
    var res1 = con.query("select * FROM session where s_token=?", [schedule.token], function (err, result0, fields) {
       ...
       //console.log(x)
       //console.log(result0)
        var val = [schedule.val, schedule.unit, schedule.operation, schedule.importance, result0[0].s_user_id];
        if (result0[0]) {
            var res = con.query("INSERT INTO notification (nt_value,nte_id,nt_operation,nt_importance,nt_user) VALUES ?", [[val]], function (err, result, fields) {
               //console.log(res.sql);
                //console.log(result); 
