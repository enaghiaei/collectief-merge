...
# Method to insert a single row into the table
    def insert(self, item):
        ...
        #self.con.commit()
        print("222222222222222111");
        return self.cur.lastrowid

# Method to insert multiple rows into the table
    def insert_many(self, items):
        print("2222222222222223333111");
        table_query =  """INSERT INTO {}(timestamp, sensor_type, channel, measure_name, measure_var,measure_kind, is_external, is_calibrated, measure_value, sensor_serial) VALUES(?,?,?,?,?,?,?,?,?,?)""".format(self.table_name)
        sql =  "INSERT INTO node_220403690 (timestamp, sensor_type, channel, measure_name, measure_var,measure_kind, is_external, is_calibrated, measure_value, sensor_serial) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        self.mycursor.executemany(sql, items)
        self.mydb.commit()
        #self.cur.executemany(table_query, items)
        #self.con.commit()
        print("2222222222222223333");
