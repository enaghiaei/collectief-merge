...
class Edit_users extends React.Component {
    ...
    users() {
        ...
        return fetch('http://' + global.config.vals.root.ip + ':3002/get_users_detail', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ token: cookies.get('token'), user_token: this.state.user_token })
        })
            .then(data => data.json())
            .then(
                (result) => {
                    ...
                    for (var key in result.result) {
                        location_list[location_list.length] = result.result[key].location_id;
                    }

                    $("#username").val(result.result[0]["email"]);
                    $("#phone").val(result.result[0]["phone_number"]);
                    $("#name").val(result.result[0]["fullname"]);
                    $("#user_type").val(result.result[0]["user_type"]);
                    this.setName(result.result[0]["fullname"])
                    this.setPhone(result.result[0]["phone_number"])
                    this.setUserName(result.result[0]["email"])
                    this.setState({
    ...
    create_user() {
        ...
        if (this.checkForm()) {
            ...
            return fetch('http://' + global.config.vals.root.ip + ':3002/edit_user', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    token: cookies.get('token'), password: this.state.password, name: this.state.name, phone: this.state.phone, user_type: this.state.user_type, location_id: this.state.location_id, username: this.state.username, user_token: this.state.user_token
                })
            })
                .then(data => data.json())
                .then(
    ...
    renderLocationValues() {
        ...
        return this.state.location_types_parent.map(function (o, i) {
            // var x = context.state.sensor_info[i].p_created_at.split("T");
            if (context.state.location_list.includes(context.state.location_types_parent[i].id)) {
                return (
                    <option value={context.state.location_types_parent[i].id} selected> {context.state.location_types_parent[i].title}</ option>
                );
            } else {
                return (
    ...
    setLocation(id) {
        ...
        this.setState({
            "location_id": checkedItems,
            "location_list": checkedItems
    ...
    renderLocationValues() {
        ...
        return this.state.location_types_parent.map(function (o, i) {
            ...
            if (context.state.location_list.includes(context.state.location_types_parent[i].id)) {
                return (
                    <div className="check_box_content" value={context.state.location_types_parent[i].id}><input className="check_box_item" type="checkbox" checked onClick={(event) => context.setLocation(context.state.location_types_parent[i].id)} id={"check_" + context.state.location_types_parent[i].id} value={context.state.location_types_parent[i].id} />&nbsp;&nbsp;&nbsp;{context.state.location_types_parent[i].title}</div>
                );
            } else {
                return (
                    ...
                    <div className="main_content text-left">
                        ...
                        <div className="main_content_right">
                            <div className="user_info  ml-4">
                                <div className="user_info_item d-none">
                                ...
                                <div className="user_info_item location_ids">
                                    <div className="user_title">
                                        Location
                                    </div>
                                    <div className="newline"></div>
                                ...
                                <div className="user_info_item">
                                    ...
                                    <div className="newline"></div>
                                    <input type="text" id="username" required onChange={(e) => (this.setUserName(e.target.value))} placeholder="Enter Your username" className="user_text"></input>
                                </div>
                                <div className="newline"></div>
                                ...
                                <div className="user_info_item">
                                    ...
                                    <div className="newline"></div>
                                    <input type="password" id="password" required onChange={(e) => (this.setPassword(e.target.value))} placeholder="Enter Your password" className="user_text"></input>
                                </div>
                                <div className="newline"></div>
                                ...
                                <div className="user_info_item">
                                    ...
                                    <div className="newline"></div>
                                    <input type="text" required id="name" onChange={(e) => (this.setName(e.target.value))} placeholder="Your name" className="user_text"></input>
                                </div>
                                <div className="newline"></div>
                                ...
                                <div className="user_info_item">
                                    ...
                                    <div className="newline"></div>
                                    <input type="text" id="phone" required placeholder="Enter phone number" onChange={(e) => (this.setPhone(e.target.value))} className="user_text"></input>
                                </div>
                                <div className="newline"></div>
