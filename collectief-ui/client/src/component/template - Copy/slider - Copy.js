...
import { faAdd, faMinus, faGear, faTemperature0, faPowerOff, faArrowRight, faArrowLeft, faInfo, faCircleInfo, faInfoCircle, faHome, faClock, faVolumeControlPhone, faWarning, faExchange, faList, faPlus, faUser } from '@fortawesome/free-solid-svg-icons';
const menus = [{ title: "Dashboard", icon: <FontAwesomeIcon icon={faHome} className="arrow3" />, icon_b: <FontAwesomeIcon icon={faHome} className="arrow3-b" />, uri: "/home" }, { title: "Scheduling", uri: "/schedule", icon: <FontAwesomeIcon icon={faClock} className="arrow3" />, icon_b: <FontAwesomeIcon icon={faClock} className="arrow3-b" /> }, { title: "Sensors info", uri: "/sensors/list_sensors", icon: <FontAwesomeIcon icon={faInfo} className="arrow3" />, icon_b: <FontAwesomeIcon icon={faInfo} className="arrow3-b" />, submenues: [{ "title": "List", "icon": <FontAwesomeIcon icon={faList} className="arrow4" />, uri: "/sensors/list_sensors", "id": "2" }, { "title": "Create", uri: "/sensors/locations", "icon": <FontAwesomeIcon icon={faPlus} className="arrow4" />, "id": "3" }, { "title": "Assign", uri: "/sensors/assignment", "icon": <FontAwesomeIcon icon={faExchange} className="arrow4" />, "id": "4" }] }, { title: "Users", uri: "/sensors/list_sensors", icon: <FontAwesomeIcon icon={faUser} className="arrow3" />, icon_b: <FontAwesomeIcon icon={faUser} className="arrow3-b" />, submenues: [ { "title": "List", uri: "/users/list_users", "icon": <FontAwesomeIcon icon={faPlus} className="arrow4" />, "id": "9" }, { "title": "Create", uri: "/users/create_users", "icon": <FontAwesomeIcon icon={faExchange} className="arrow4" />, "id": "10" }] }, { title: "Send Feedback", uri: "/control", icon: <FontAwesomeIcon icon={faVolumeControlPhone} className="arrow3" />, icon_b: <FontAwesomeIcon icon={faVolumeControlPhone} className="arrow3-b" /> },  { title: "Notification", uri: "/notification", icon: <FontAwesomeIcon icon={faWarning} className="arrow3" />, icon_b: <FontAwesomeIcon icon={faWarning} className="arrow3-b" /> }, { title: "Logout", uri: "", icon: <FontAwesomeIcon icon={faPowerOff} className="arrow3" />, icon_b: <FontAwesomeIcon icon={faPowerOff} className="arrow3-b" /> }];
...
class Slider extends React.Component {
  ...
  componentDidMount(){
    ...
    resizeCanvas() {
        ...
        if (page_size.width > 1056) {
            ...
            $(".v_m").css("height", (0.2 * page_size.height));
        ...
        if (menus[id].submenues) {
            return menus[id].submenues.map(function (o, key) {
                ...
                if (context.state.menu_id == id) {
                    return (
                        <div className="ml-3 mt-2" onClick={(event) => context.choose_menu2(menus[id].submenues[key].id)}>
                    ...
                    return (
                        <div className="ml-3 mt-2 d-none" onClick={(event) => context.choose_menu2(menus[id].submenues[key].id)}>
                            <div style={{ "display": "inline-table" }}>
                                {menus[id].submenues[key].icon}
                            </div>
                            <div style={{ "display": "inline-table" }} className="ml-4" class={context.state.isActive ? 'menu-item-text d-none' : 'menu-item-text ml-3'}>
  ...
  set_menu(id){
    ...
    console.log("indexOf==="+window.location.pathname.indexOf(menus[id].title.toLocaleLowerCase()));
      ...
      return (<div class="menu-item border-right-n" title={menus[id].title} >
